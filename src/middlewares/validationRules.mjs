...
minima de 3 caracteres y una longitud maxima de 60*/
    body('nombreReal')
    //.exists()
    .trim().notEmpty().withMessage('El nombre Real es requerido')
...
valor minimo 0 (no admite edad negativa)*/
    body('edad')
    //.exists()
    .trim().notEmpty().withMessage('Edad es requerida')
...
y una longitud maxima de 60*/
    ...
    body('poderes.*')
    //.exists()
    .trim().isString().withMessage('Cada poder debe ser una cadena de texto')
